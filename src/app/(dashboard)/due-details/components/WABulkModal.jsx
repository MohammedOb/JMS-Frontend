...
export default function WABulkModal({ open, onClose, rows = [] }) {
  ...
  const [queued,      setQueued]      = useState(null); // { batchId, total } after success
  ...
  async function handleQueue() {
    ...
    // Build per-member items with pre-rendered messages
    ...
    try {
      const res = await waQueueService.create({
        label: `Due Reminder — ${rows.length} member${rows.length !== 1 ? 's' : ''}`,
        items,
      });
      if (res.data?.success) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Send WhatsApp Reminders — ${rows.length} member${rows.length !== 1 ? 's' : ''}`}
      size="xl"
      footer={
        queued ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Write your message using {FullName}, {DueAmount} etc."
            />
