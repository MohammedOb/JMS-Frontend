...
function fmtDate(value) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) {
    ...
    const p = new Date(s);
    d = isNaN(p) ? null : new Date(p.getFullYear(), p.getMonth(), p.getDate());
  }
...
function fmtDateTime(value) {
  ...
  return `${d.getDate()} ${M[d.getMonth()]} ${d.getFullYear()}, ${h % 12 || 12}:${m} ${h >= 12 ? 'pm' : 'am'}`;
}
...
export default function EditEventModal({ open, onClose, form, onChange, onSave, saving }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Edit Event${form.eventName ? ` — ${form.eventName}` : ''}`}
      size="lg"
      footer={
        <>
    ...
    >
      ...
      {/* ── Top info bar ── */}
      <div className="flex flex-wrap gap-5 bg-blue-50 rounded-lg px-4 py-3 border border-blue-100 mb-1">
        <div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Serial No</div>
          <div className="text-[15px] font-bold text-red-500">{form.serialNo || '—'}</div>
        </div>
        <div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Request Date</div>
          <div className="text-[13px] font-semibold text-navy-800">{fmtDate(form.requestDate)}</div>
        </div>
        <div>
        ...
        <div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Requested By</div>
          <div className="text-[13px] font-semibold text-navy-800">{form.requestBy || '—'}</div>
        </div>
        <div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Created By</div>
          <div className="text-[13px] font-semibold text-navy-800">{form.createdBy || '—'}</div>
        </div>
      </div>
      ...
      <div className="space-y-4 text-[13px]">
        ...
        <div>
          ...
          <div className="mb-3">
            ...
            <input
              ...
              placeholder="Type Acc No to re-fill member details"
              value={accNo}
        ...
        {/* 5. Update Record */}
        <div className="border border-amber-200 rounded-lg bg-amber-50/40 px-4 py-3">
          <div className="text-[11px] font-bold uppercase tracking-widest text-amber-600 mb-3">Update Record</div>
          {(form.updateReason || form.updatedAt) && (
            <div className="grid grid-cols-2 gap-4 mb-3 pb-3 border-b border-amber-100 text-[12px]">
              <div>
                <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-1">Last Update Reason</div>
                <div className="text-gray-600">{form.updateReason || '—'}</div>
              </div>
              <div>
                <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-1">Last Update Date</div>
                <div className="text-gray-600">{fmtDateTime(form.updatedAt)}</div>
              </div>
            </div>
          )}
          <div>
            ...
            <input
              ...
              onChange={e => onChange('reasonForUpdate', e.target.value)}
            />
          </div>
        </div>
