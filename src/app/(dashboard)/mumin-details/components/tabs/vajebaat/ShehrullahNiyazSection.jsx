...
export default function ShehrullahNiyazSection({ sniyazList, onAddSniyaz, onSniyazForm, onEditSniyaz, onDeleteSniyaz, sniyazFormButton }) {
  ...
  const sorted = [...sniyazList].sort(byYearDesc);
    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2 text-[11px] font-semibold text-gray-400 uppercase tracking-wider flex-1 min-w-0">
          <span>Shehrullah Niyaz Takhmeen</span>
      ...
      <div className="overflow-x-auto rounded-lg border border-border mb-4">
        <table className="w-full border-collapse text-[12px] min-w-[360px]">
          ...
          <tbody>
            {paginated.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-6 text-gray-400">No S. Niyaz records</td></tr>
            ) : paginated.map((s, i) => (
                  ...
                  <div className="flex items-center gap-1">
                    {onEditSniyaz && (
                      <button
                        ...
                        onClick={() => onEditSniyaz({ ...s, date: toInputDate(s.date) })}
                      >
