      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-white">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-[12px] font-semibold text-gray-800 truncate">
            {batch.label || 'Takhmeen Reminder Batch'}
          </div>
          <div className="text-[10px] text-gray-400">
            {fmtTime(batch.created_at)} · {batch.created_by || '—'}
            {batch.processing_name && isActive && (
              <span className="ml-2 text-blue-500 font-medium">⌨️ {batch.processing_name}</span>
            )}
          </div>
        </div>
        <div className="text-[11px] text-gray-600 whitespace-nowrap">{done}/{total}</div>
        <StatusBadge status={batch.status} />
        {isActive && pending > 0 && (
          <div className="text-[10px] text-gray-400 whitespace-nowrap">{etaStr(pending)}</div>
        )}
    ...
    <div className="mb-4 border border-border rounded-lg overflow-hidden shadow-sm">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-semibold">📤 WhatsApp Queue</span>
          {activeCount > 0 ? (
